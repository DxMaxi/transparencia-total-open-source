...
test("the release separates intentional visibility from the unresolved privacy gate", async () => {
  ...
  assert.match(audit, /Decisão posterior — 7 de setembro de 2026/);
  assert.match(audit, /tornou o repositório público de forma\s+intencional/i);
  assert.match(audit, /resultado histórico de privacidade\s+permanece `FAIL`/i);
