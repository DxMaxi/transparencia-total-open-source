...
from app.models.editorial import PoliticianMandateWithdrawalRequest
...
def _payload(**overrides: object) -> PoliticianMandateWithdrawalRequest:
    values: dict[str, object] = {
        "expected_case_id": "case_alpha",
        "expected_revision": 4,
        "expected_version_id": "version_alpha",
        "expected_version_sha256": "a" * 64,
        "expected_mandate_id": "mandate_alpha",
    ...
    return PoliticianMandateWithdrawalRequest.model_validate(values)


def test_mandate_withdrawal_request_is_strict_and_requires_all_confirmations() -> None:
    valid = _payload()
    assert valid.confirm_withdrawal is True
    assert valid.rationale.startswith("A fonte")

    with pytest.raises(ValidationError):
        _payload(confirm_mandate_and_history_preserved=False)
    with pytest.raises(ValidationError):
...
def test_mandate_withdrawal_confirmation_rejects_every_stale_proof() -> None:
    PoliticianMandateWithdrawalRepository._confirm_payload(
        case_id="case_alpha",
        preview=_preview(),
        payload=_payload(),
    )
    ...
    for field, changed in (
        ...
        ("expected_mandate_id", "mandate_beta"),
