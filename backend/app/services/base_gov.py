...
def _public_identifier(value: Any | None) -> str | None:
    text = _as_text(value)
    if not text:
        return None
...
class BaseGovCollector:
    ...
    async def collect(self, year: int, *, limit: int | None = None) -> BaseContractCollection:
        ...
        for raw in self.iter_records(response.content, resource.format):
            ...
            if contract is None:
                skipped += 1
                continue
            if contract.direct_official_url is None:
                without_direct_link += 1
    ...
    def normalise_contract(
        ...
        *,
        ...
        direct_url = (
            direct_url_text if direct_url_text and is_official_url(direct_url_text) else None
        )
        source_url = direct_url or dataset_url
        source = OfficialSource(
            publisher=SourcePublisher.BASE_GOV,
            label="Portal BASE — contrato público",
    ...
    @staticmethod
    def _parties(value: Any | None, role: ContractPartyRole) -> list[PublicContractParty]:
        if value is None:
            return []
        if isinstance(value, list):
            result: list[PublicContractParty] = []
            for item in value:
                result.extend(BaseGovCollector._parties(item, role))
            return result
        if isinstance(value, dict):
            ...
            return [
                PublicContractParty(
                    name=name,
                    public_identifier=_public_identifier(
        ...
        if not text:
            return []
        return [
            PublicContractParty(name=name.strip(), role=role)
...
class ContractMatcher:
    ...
    def match(
        ...
        name_index = {normalise_public_name(actor.public_name): actor for actor in eligible}
        protected_index: dict[str, PublicActorMatchKey] = {}
        association_id_index: dict[str, tuple[PublicActorMatchKey, HttpUrl]] = {}
        association_name_index: dict[str, tuple[PublicActorMatchKey, HttpUrl]] = {}
        for eligible_actor in eligible:
            if self.pepper and eligible_actor.protected_nif is not None:
                digest = hmac_protected_identifier(
                    eligible_actor.protected_nif.get_secret_value(),
                    self.pepper,
                )
                protected_index[digest] = eligible_actor
            for actor_association in eligible_actor.official_associations:
                ...
                if not association_has_public_evidence(evidence):
                    continue
                if actor_association.public_nipc:
                    association_id_index[actor_association.public_nipc] = (
                        eligible_actor,
                        actor_association.official_evidence_url,
                    )
                association_name_index[
                    normalise_public_name(actor_association.organisation_name)
                ] = (eligible_actor, actor_association.official_evidence_url)

        matches: dict[tuple[str, str, str, str], ContractMatchCandidate] = {}
        for contract in contracts:
            parties = [*contract.contracting_authorities, *contract.contractors]
            for party in parties:
                matched_actor: PublicActorMatchKey | None = None
                method: ContractMatchMethod | None = None
                score = Decimal("0")
                association_evidence: HttpUrl | None = None
                if party.public_identifier and self.pepper:
                    digest = hmac_protected_identifier(party.public_identifier, self.pepper)
                    matched_actor = protected_index.get(digest)
                    if matched_actor:
                        method = ContractMatchMethod.EXACT_PROTECTED_IDENTIFIER
                        score = Decimal("1.0000")
                if matched_actor is None and party.public_identifier:
                    association_match = association_id_index.get(party.public_identifier)
                    if association_match:
                        matched_actor, association_evidence = association_match
                        method = ContractMatchMethod.EXACT_PUBLIC_ORGANISATION_ID
                        score = Decimal("1.0000")
                normalised_party = normalise_public_name(party.name)
                if matched_actor is None:
                    association_match = association_name_index.get(normalised_party)
                    if association_match:
                        matched_actor, association_evidence = association_match
                        method = ContractMatchMethod.NORMALISED_NAME
                        score = Decimal("0.9500")
                if matched_actor is None:
                    matched_actor = name_index.get(normalised_party)
                    if matched_actor:
                        method = ContractMatchMethod.NORMALISED_NAME
                        score = Decimal("0.9000")
                if matched_actor is None or method is None:
                    continue
                contract_source = contract.direct_official_url or contract.source.url
                candidate = ContractMatchCandidate(
                    ...
                    method=method,
                    score=score,
                    contract_source_url=contract_source,
