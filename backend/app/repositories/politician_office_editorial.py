"""Porta privada entre cargos oficiais observados e a revisão humana."""
...
_INGESTION_ALIAS = "parliament-office-ingestion"
_SUBJECT_TYPE = "PARLIAMENT_OFFICE_PERIOD"
_SCHEMA_VERSION = "politician-office-editorial-v1"
...
def _parse_date(value: object) -> datetime | None:
    ...
    if not isinstance(value, str) or not value.strip():
        raise ValueError("Uma data de cargo deixou de ter o formato textual esperado")
    try:
        parsed = datetime.fromisoformat(value.replace("Z", "+00:00"))
    except ValueError as exc:
        raise ValueError("Uma data de cargo deixou de ser ISO-8601 válida") from exc
...
def _json_object(value: object) -> dict[str, Any]:
    ...
    if not isinstance(decoded, dict):
        raise ValueError("O cargo parlamentar deixou de ser um objeto")
    return dict(decoded)
...
class PoliticianOfficeEditorialRepository:
    """Reconstrói cargos por intervalo e cria apenas processos privados ``PENDING``."""
    ...
    async def list_candidates(
        ...
        *,
        ...
        return {
            ...
            "search_rule": (
                "A pesquisa limita cargos já ligados a um DepId e CarId oficiais; não associa "
                "pessoas por nome nem transforma o cargo observado num mandato ou filiação."
            ),
        }
        ...
        *,
        ...
        if candidate is None:
            raise EditorialSourceError(
                "O cargo oficial não existe ou deixou de corresponder à observação atestada"
            )
        if candidate["proposal_eligible"] is not True:
            ...
            raise EditorialSourceError(
                "O cargo não reúne prova suficiente para revisão humana específica"
        ...
        case, created = await self.editorial.create_ingestion_case(
            ...
            submission_rationale=(
                "Cargo parlamentar oficial enviado para revisão privada; DepId, CarId, período "
                "e círculo foram confirmados por identificadores exatos, mas não se inferiu "
                "mandato, filiação, continuidade jurídica ou publicação."
            ),
            actor=actor,
        )
        return {
    ...
    async def get_exact_candidate(
        ...
        *,
        ...
        """Reconstrói um cargo exato; o hash nunca é aceite como conteúdo do cliente."""
        ...
        *,
        ...
        if query:
            ...
            conditions.append(
                ...
                f"OR office.period ->> 'title' ILIKE ${search_arg} ESCAPE '!')"
            )
        if observation_id:
        ...
        rows = await database.fetch(
            ...
            SELECT observation.id AS observation_id,
                   ...
                   office.period,
                   office.ordinality::int AS period_ordinal,
                   ...
                   office_case.id AS case_id,
                   office_case.current_state AS case_state,
                   office_case.revision AS case_revision,
                   office_case.origin AS case_origin,
            ...
            CROSS JOIN LATERAL jsonb_array_elements(observation.offices)
                WITH ORDINALITY AS office(period, ordinality)
            ...
            LEFT JOIN editorial_cases AS office_case
              ON office_case.kind = 'POLITICIAN_PROFILE'::"EditorialCaseKind"
             AND office_case.subject_type = '{_SUBJECT_TYPE}'
             AND office_case.subject_id = observation.id || ':' || office.ordinality::text
             AND office_case.source_document_id = snapshot.source_document_id
    ...
    def _candidate(row: Mapping[str, Any]) -> dict[str, object]:
        blocked: list[str] = []
        warnings = [
            "O cargo é uma observação oficial da ficha parlamentar; a revisão não prova "
            "competências, continuidade ou efeitos jurídicos além do intervalo fornecido.",
            "A ausência de CarId, data ou círculo significa dados indisponíveis.",
        ]
        try:
            period = _json_object(row["period"])
            if set(period) - {"source_id", "title", "starts_at", "ends_at"}:
                raise ValueError("O cargo contém campos fora do contrato V5.27")
            office_source_id = period.get("source_id")
            if office_source_id is not None and (
                not isinstance(office_source_id, str) or not office_source_id.strip()
            ):
                raise ValueError("O identificador oficial do cargo deixou de ser válido")
            title = period.get("title")
            if not isinstance(title, str) or not title.strip():
                raise ValueError("O cargo perdeu a designação oficial")
            starts_at = _parse_date(period.get("starts_at"))
            ends_at = _parse_date(period.get("ends_at"))
        except (TypeError, ValueError, json.JSONDecodeError) as exc:
            office_source_id = None
            title = "Dados indisponíveis"
            starts_at = ends_at = None
            blocked.append(f"Estrutura do cargo inválida: {exc}")

        normalized_period = {
            "source_id": office_source_id.strip() if isinstance(office_source_id, str) else None,
            "title": title.strip(),
        ...
        if normalized_period["source_id"] is None:
            blocked.append("O cargo não tem CarId oficial e não pode originar uma associação.")
        if starts_at is None:
            blocked.append("A data oficial de início do cargo está indisponível.")
        ...
        return {
            ...
            "source_office": normalized_period,
            "source_period_sha256": source_period_sha256,
            "snapshot": {
    ...
    def _normalized_proposal(candidate: dict[str, object]) -> dict[str, Any]:
        ...
        return {
            ...
            "office_candidate": {
                ...
                "source_office": {
                    "official_id_reference_sha256": _reference_sha256(source_office["source_id"]),
                    "title": source_office["title"],
                    "starts_at": source_office["starts_at"],
                    "ends_at": source_office["ends_at"],
                },
                "source_period_sha256": candidate["source_period_sha256"],
                "meaning": "OBSERVED_OFFICIAL_OFFICE_PERIOD_REQUIRING_HUMAN_REVIEW",
                "constituency": {
            ...
            "publication": {
                ...
                "office_creation_performed": False,
