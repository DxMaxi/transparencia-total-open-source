...
class EditorialCaseKind(StrEnum):
    ...
    ORGANISATION_PUBLICATION = "ORGANISATION_PUBLICATION"
...
class PoliticianInitiativeAuthorshipPublicationRequest(BaseModel):
    """Confirma uma autoria reconstruída no servidor por IniId e idCadastro exatos."""
    ...
    expected_source_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    expected_source_record_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    expected_activity_snapshot_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    expected_publication_proof_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    rationale: str = Field(min_length=20, max_length=1850)
    public_rationale: str = Field(min_length=20, max_length=500)
    confirm_source_reviewed: Literal[True]
    confirm_exact_official_ids_only: Literal[True]
    confirm_official_author_relation: Literal[True]
    confirm_public_initiative_reviewed: Literal[True]
    confirm_no_name_or_party_matching: Literal[True]
    confirm_no_collective_position_inference: Literal[True]
    confirm_append_only_publication: Literal[True]
    confirm_publication: Literal[True]
    ...
    @field_validator("rationale", "public_rationale")
    @classmethod
    def strip_initiative_authorship_publication_rationale(cls, value: str) -> str:
        stripped = value.strip()
        if len(stripped) < 20:
            raise ValueError("A fundamentação deve ter pelo menos 20 caracteres úteis")
        return stripped
...
class PoliticianInitiativeAuthorshipWithdrawalRequest(BaseModel):
    """Retira uma autoria exata sem alterar a ligação nem a publicação original."""
    ...
    expected_version_id: str = Field(pattern=r"^[A-Za-z0-9_-]{1,200}$")
    expected_version_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    expected_authorship_id: str = Field(pattern=r"^[A-Za-z0-9_-]{1,200}$")
    expected_source_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    expected_source_record_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    expected_activity_snapshot_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    expected_publication_proof_sha256: str = Field(pattern=r"^[0-9a-f]{64}$")
    ...
    confirm_exact_authorship: Literal[True]
    confirm_public_effect_reviewed: Literal[True]
    confirm_authorship_and_history_preserved: Literal[True]
    confirm_no_identity_initiative_or_party_change: Literal[True]
    confirm_no_vote_or_collective_position_inference: Literal[True]
    confirm_withdrawal: Literal[True]

    @field_validator("rationale", "public_rationale")
    @classmethod
    def strip_initiative_authorship_withdrawal_rationale(cls, value: str) -> str:
        stripped = value.strip()
        if len(stripped) < 20:
            raise ValueError("A fundamentação deve ter pelo menos 20 caracteres úteis")
        return stripped
...
class PoliticianAttendanceWithdrawalRequest(BaseModel):
    """Retira uma reunião integral sem apagar a sessão ou qualquer presença."""
    ...
    confirm_complete_meeting: Literal[True]
    confirm_public_effect_reviewed: Literal[True]
    confirm_session_records_and_history_preserved: Literal[True]
    confirm_no_selective_person_or_mandate_change: Literal[True]
    confirm_absence_is_not_noncompliance: Literal[True]
    confirm_withdrawal: Literal[True]

    @field_validator("rationale", "public_rationale")
    @classmethod
    def strip_attendance_withdrawal_rationale(cls, value: str) -> str:
        stripped = value.strip()
        if len(stripped) < 20:
            raise ValueError("A fundamentação deve ter pelo menos 20 caracteres úteis")
        return stripped
