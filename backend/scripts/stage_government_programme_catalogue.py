"""Arquiva e prepara o catálogo integral do Programa do Governo apenas em staging."""
...
async def stage(*, actor_alias: str, confirmed: bool) -> dict[str, object]:
    ...
    async with OfficialHttpClient(settings) as http:
        response = await http.get(str(manifest.source_url), max_bytes=10_000_000)
    if str(response.url) != str(manifest.source_url):
        raise RuntimeError("A URL final do Programa do Governo diverge do manifesto revisto")
    mime_type = response.headers.get("content-type", "").split(";", 1)[0].strip().lower()
    if mime_type != "application/pdf":
        raise RuntimeError("A fonte oficial não devolveu um PDF")

    catalogue = extract_government_programme_catalogue(
    ...
    repository = GovernmentProgrammeStagingRepository(settings)
    await repository.connect()
    try:
        schema_readiness = await repository.require_catalogue_schema()
        receipt = await repository.archive_raw_document(raw_document=raw_document)
        report = await repository.stage_catalogue(
