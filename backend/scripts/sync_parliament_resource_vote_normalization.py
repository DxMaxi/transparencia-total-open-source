"""Normaliza privadamente votações de um único arquivo JSON parlamentar."""
...
def validate_private_vote_normalization_operation(
    settings: Settings,
    *,
    confirmed: bool,
) -> None:
    ...
    if settings.environment != "staging":
        raise RuntimeError("ENVIRONMENT tem de ser staging para normalizar votações históricas")
...
async def sync(
    *,
    ...
    validate_private_vote_normalization_operation(settings, confirmed=confirmed)
    ...
    try:
        ...
        return await ParliamentResourceVoteNormalizationStager(settings, repository).store(
            collection
        )
    finally:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Normalizar votações de um único arquivo JSON, sem criar revisão",
    )
