"""Normaliza privadamente deputados de um único arquivo parlamentar JSON."""
...
def validate_private_deputy_normalization_operation(
    settings: Settings,
    *,
    confirmed: bool,
) -> None:
    ...
    if settings.environment != "staging":
        raise RuntimeError("ENVIRONMENT tem de ser staging para normalizar deputados históricos")
...
async def sync(
    *,
    ...
    validate_private_deputy_normalization_operation(settings, confirmed=confirmed)
    ...
    repository = ParliamentResourceDeputyNormalizationRepository(settings)
    ...
    try:
        ...
        return await ParliamentResourceDeputyNormalizationStager(
            settings,
            repository,
        ).store(collection)
    finally:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Normalizar deputados de um único arquivo JSON, sem criar revisão",
    )
