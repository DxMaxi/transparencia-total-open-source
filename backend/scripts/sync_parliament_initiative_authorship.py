"""Normaliza autorias individuais apenas a partir de um arquivo oficial atestado."""
...
def validate_private_authorship_operation(settings: Settings, *, confirmed: bool) -> None:
    if not confirmed:
        raise RuntimeError("A persistência exige --confirm-private-staging")
    if settings.environment != "staging":
        raise RuntimeError("ENVIRONMENT tem de ser staging para guardar autorias")
...
async def sync(
    *,
    ...
    validate_private_authorship_operation(settings, confirmed=confirmed)
    ...
    repository = ParliamentInitiativeAuthorshipRepository(settings)
    ...
    try:
        ...
        return await ParliamentInitiativeAuthorshipStager(settings, repository).store(collection)
    finally:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Normalizar iniAutorDeputados/idCadastro sem criar pessoas, revisão ou publicação"
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Confirmar staging privado, sem revisão nem publicação",
    )
