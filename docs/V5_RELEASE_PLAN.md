...
candidato V5.53, da produção pública e dos princípios de governação do projeto. Não autoriza deploy, migração remota, criação de
...
| Código V5 | V5.1 a V5.52.1 integradas; V5.53 preparada; staging remoto pendente |
...
| Investigador Cívico | V5.49–V5.53 preparam âmbito, contratos e identidade/publicação organizacional; zero associações ou relações e nenhuma operação real executada |
| PWA e alertas | consentimento, revogação e cache opt-in integrados; configuração remota pendente |
| Repositório | público por decisão intencional confirmada em 07-09-2026; licença não comercial; privacidade do contacto histórico por reconciliar |

A V5.52 foi integrada pela [PR #132](https://github.com/DxMaxi/transparencia-total-open-source/pull/132)
em `eb43279`, com CI aprovado, e a V5.52.1 está em `main`. Em 07-09-2026 o responsável confirmou
expressamente que tornar o repositório público foi intencional. Essa autorização não transforma a
auditoria V5.19 num `PASS` nem resolve por si só o contacto pessoal histórico. O
[checkpoint operacional V5.53](V5_OPERATIONAL_READINESS.md) separa o site acessível, a recolha
parlamentar suspensa, a nova porta organizacional e as operações remotas ainda pendentes.
...
O décimo quarto gate de perfis está preparado em
[V5.37 — publicação transacional de cargos parlamentares](V5_POLITICIAN_OFFICE_PUBLICATION.md).
...
O décimo nono gate de perfis está preparado em
[V5.42 — autoria individual de iniciativas no circuito editorial](V5_POLITICIAN_INITIATIVE_AUTHORSHIP.md).
Dos bytes oficiais já arquivados é derivada uma fotografia privada separada das relações
...
A [V5.52 — identidade organizacional privada com fonte própria](V5_BASE_ORGANISATION_IDENTITY.md)
prepara uma observação de registo IRN com arquivo atestado, referência não fiscal e HMAC privado
com pepper estável. A proposta dedicada cria apenas `ORGANISATION_IDENTITY/PENDING`; aprovação
continua sem organização pública, parte de contrato, correspondência ou relação. A criação e
correção genéricas não podem contornar a fonte e novas correspondências por nomes normalizados
ficam bloqueadas sem apagar o legado. O preflight recusa remover a coluna fiscal legada se
existirem valores. No checkpoint V5.52, publicação/retirada de organizações, associação exata às
partes e grafo continuavam portas próprias. Nenhuma operação remota foi executada: o ambiente GitHub `staging`
não estava configurado na inspeção read-only de 2 de setembro de 2026.

A [V5.53 — publicação e retirada específicas de organizações](V5_BASE_ORGANISATION_PUBLICATION.md)
acrescenta uma segunda proposta `ORGANISATION_PUBLICATION/PENDING`. A identidade privada aprovada
continua sem efeito público até existir revisão própria e confirmação `ADMIN` com MFA. A publicação
cria apenas uma fotografia mínima com identificador não fiscal, fonte IRN, data e hashes; NIPC,
HMAC, observação privada, partes, correspondências e relações ficam excluídos. A retirada acrescenta
histórico e preserva os direitos de resposta. A associação exata de partes e o grafo continuam em
portas posteriores e nenhuma migração ou publicação real foi executada.
